...
export function EmployeesClient() {
  ...
  async function onSubmit(formData: z.infer<typeof UploadEmployeeSchema>) {
    ...
    try {
      ...
      const employees = jsonData.map((row, index) => parseEmployee(row, index));
    ...
    <>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-x-4">
          <button type="button" onClick={handleExportExcel} className="ds-btn-primary">
            <Download className="size-4" />
            Exporter Excel
          </button>
          <button type="button" onClick={() => setIsFileUploadOpen(true)} className="ds-btn-primary">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="relative w-full max-w-xs">
          ...
          />
          {searchInput && (
            <button type="button" onClick={() => setSearchInput("")} className="absolute right-2 top-1/2 -translate-y-1/2 rounded p-0.5 transition-colors" style={{ color: "var(--muted)" }}>
      ...
      <div className="flex flex-wrap items-end gap-3">
        <div className="flex flex-col gap-1">
          ...
          <select value={filterProductionLine} onChange={(e) => { setFilterProductionLine(e.target.value); setPage(0); }} className="h-9 rounded-lg border text-sm outline-none px-2" style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--text)", minWidth: 160 }}>
        ...
        <div className="flex flex-col gap-1">
          ...
          <select value={filterShift} onChange={(e) => { setFilterShift(e.target.value); setPage(0); }} className="h-9 rounded-lg border text-sm outline-none px-2" style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--text)", minWidth: 130 }}>
        ...
        <div className="flex flex-col gap-1">
          ...
          <select value={filterEmploymentType} onChange={(e) => { setFilterEmploymentType(e.target.value); setPage(0); }} className="h-9 rounded-lg border text-sm outline-none px-2" style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--text)", minWidth: 150 }}>
            <option value="">Tous</option>
        ...
        <div className="flex flex-col gap-1">
          ...
          <input type="date" value={filterHireDateFrom} onChange={(e) => { setFilterHireDateFrom(e.target.value); setPage(0); }} className="h-9 rounded-lg border text-sm outline-none px-2" style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--text)", minWidth: 150 }} />
        </div>

        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium" style={{ color: "var(--text2)" }}>Embauche — fin</label>
          <input type="date" value={filterHireDateTo} onChange={(e) => { setFilterHireDateTo(e.target.value); setPage(0); }} className="h-9 rounded-lg border text-sm outline-none px-2" style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--text)", minWidth: 150 }} />
        </div>

        {hasActiveFilters && (
          <button type="button" onClick={resetFilters} className="h-9 flex items-center gap-1.5 px-3 rounded-lg border text-sm" style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--accent4)" }}>
            <X className="size-3.5" />
            Réinitialiser
          </button>
        )}
      ...
      <DataTable columns={columns} data={formattedEmployees} initialPageSize={PAGE_SIZE} hidePagination />
      ...
      <div className="flex items-center justify-between px-1 pt-2">
        ...
        <div className="flex items-center gap-1">
          <button type="button" onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={isFirst || isFetching} className="flex h-8 w-8 items-center justify-center rounded-md border transition-colors disabled:opacity-40" style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--text2)" }}>
          ...
          {Array.from({ length: totalPages }, (_, i) => i)
            ...
            .map((i) => (
              <button key={i} type="button" onClick={() => setPage(i)} disabled={isFetching} className="flex h-8 w-8 items-center justify-center rounded-md border text-sm font-medium transition-colors disabled:opacity-40"
                style={{ background: i === page ? "var(--accent)" : "var(--surface2)", border: `1px solid ${i === page ? "var(--accent)" : "var(--border)"}`, color: i === page ? "#fff" : "var(--text2)" }}>
                {i + 1}
              </button>
            ))}

          <button type="button" onClick={() => setPage((p) => Math.min(totalPages - 1, p + 1))} disabled={isLast || isFetching} className="flex h-8 w-8 items-center justify-center rounded-md border transition-colors disabled:opacity-40" style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--text2)" }}>

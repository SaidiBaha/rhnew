...
function getTodayTunis(): string {
  // en-CA produit systématiquement le format YYYY-MM-DD
 ...
 */
export function parseNewAttendanceFormat(jsonData: unknown[]): AttendanceRequest[] {
  type GroupEntry = {
    matricule: string;
    date: string;
    clockIn: string | null;
  ...
  for (let i = 0; i < jsonData.length; i++) {
    ...
    const rowNum = i + 2; // ligne Excel (1 = headers)
    ...
    // Les cellules numériques Excel arrivent sous forme "12345.0" avec raw:false → on retire le .0

...
public class AttendanceService {
    ...
    public void batchSave(List<SaveAttendanceInputDto> attendanceInputs) {
        ...
        Set<String> employeeMatricules = attendanceInputs
                .stream()
                .map(SaveAttendanceInputDto::getMatricule)
                .collect(Collectors.toSet());

        LocalDate minDate = attendanceInputs
                .stream()
                .map(SaveAttendanceInputDto::getDate)
                .min(LocalDate::compareTo)
                .orElseThrow();

        LocalDate maxDate = attendanceInputs
                .stream()
                .map(SaveAttendanceInputDto::getDate)
        ...
        Map<String, Attendance> attendanceMap = existingAttendances
                .stream()
                .collect(Collectors
                        .toMap(a -> generateAttendanceKey(a.getEmployee().getMatricule(), a.getDate()),
                                a -> a)
                );
        ...
        for (SaveAttendanceInputDto attendanceInput : attendanceInputs) {
            ...
            if (attendance.getId() == 0) {
                ...
                if (employee == null) {
                    throw new EntityNotFoundException("Employee with matricule: " + attendanceInput.getMatricule() + " not found");
                }
